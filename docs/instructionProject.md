...
english/data/results/result_YYYY-MM-DD_INDEX.json
...
- create the JSON file in data/results/
...
data/errors.json may be empty at the beginning.
That is valid.
...
Codex must read the `errors` array from the result file and merge those records into `data/errors.json`.
...
When Codex works with `data/vocabulary/*.json`, it must also treat low-quality synonym placeholders as data issues.
