...
1) CODEX
   ...
   - selects vocabulary
   - adds grammar
   - includes mistakes from `data/errors.json`
...
3) CODEX
   ...
   - updates `data/learning.json`
   - updates `data/errors.json`
...
- if the user confirms, Codex should first save them into the appropriate `data/vocabulary/*.json` files with the relevant metadata
...
- words MUST NOT already exist in `data/learning.json`
...
- before using selected words, phrases, phrasal verbs, or idioms, Codex must verify that their records in `data/vocabulary/*.json` are correct and usable
...
- whenever Codex fixes synonym explanations, it must save the improved version into the relevant `data/vocabulary/*.json` file and also update the corresponding HTML vocabulary page so the better explanation is visible there too
...
From `data/learning.json` select:

- 20 words with the oldest date in the `next` attribute

### Procedure

1. Sort `data/learning.json` by `next` in ascending order.
...
`english/data/results/result_YYYY-MM-DD.json`
...
`data/errors.json` contains a list of the student's language mistakes.
...
If the mistake already exists in `data/errors.json`:
...
2. update `data/learning.json` (also based on `error.json`)
...
As soon as a new file `data/results/result_YYYY-MM-DD*.json` appears, Codex must immediately, without any further prompt:

- find the corresponding lesson file with the same date and index
- write new and reviewed words into `data/learning.json`
- recalculate `result`, `reviews`, `last`, and `next`
- transfer the `errors` array from the result file into `data/errors.json`
- if the `errors` field is missing or empty, Codex must not invent specific mistakes
- when creating the next lesson, Codex should create `review_errors` from `data/errors.json` for the same type of mistake, not by literally repeating the same sentence
...
`english/html/script/generate_html.ps1`
...
- after every new file written to `data/results/`, HTML regeneration must be run
- after every change to `data/learning.json`, `data/errors.json`, or `data/grammar.json`, HTML regeneration must be run
- when Codex adds or edits JSON files in `data/vocabulary/`, `data/lessons/`, or `data/results/`, it must then run HTML regeneration
...
- when Codex adds new words to `data/vocabulary/*.json`, it must also ensure the daily HTML overview based on the `inserted` field
